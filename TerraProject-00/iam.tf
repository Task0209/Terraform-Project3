...
# Create IAM Roles for AWS Backup
resource "aws_iam_role" "impex-aux-iam-backup-role" {
  name               = "impex-aux-iam-backup-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
# Create AWS Policy for Backup Role
resource "aws_iam_role_policy_attachment" "impex-aux-backup-policy" {
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSBackupServiceRolePolicyForBackup"
  role       = aws_iam_role.impex-aux-iam-backup-role.name
}